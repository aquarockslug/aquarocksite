...
function debugRender() {
			...
			if (gamepad) {
				...
				for (let j = gamepad.buttons.length; j--; ) {
					const drawPos = centerPos.add(
						vec2(j * buttonScale * 2, i * stickScale * 3 - stickScale - buttonScale),
					);
	...
	if (debugObject) {
		...
		drawTextScreen(debugText, mousePosScreen, 24, rgb(), 0.05, undefined, 'center', 'monospace');
		mainContext = saveContext;
	}
...
function randInCircle(radius = 1, minRadius = 0) {
	return radius > 0 ? randVector(radius * rand(minRadius / radius, 1) ** 0.5) : new Vector2();
}
...
class Timer {
	...
	toString() {
		if (debug) {
			return this.isSet()
				? Math.abs(this.get()) + ' seconds ' + (this.get() < 0 ? 'before' : 'after')
				: 'unset';
		}
	}
 ...
 *  @param {Number} size
 *  @memberof Settings */
function setMedalDisplayIconSize(size) {
...
class EngineObject {
	...
	update() {
		...
		if (!enablePhysicsSolver || !this.mass)
			// dont do collision for fixed objects
			return;
		...
		if (this.collideSolidObjects) {
			...
			for (const o of engineObjectsCollide) {
				...
				if (isOverlapping(oldPos, this.size, o.pos, o.size)) {
					...
					const velocity =
						length < 0.01 ? randVector(pushAwayAccel) : deltaPos.scale(pushAwayAccel / length);
...
class Sound {
	...
	play(pos, volume = 1, pitch = 1, randomnessScale = 1, loop = false) {
		...
		return (this.source = playSamples(
			this.sampleChannels,
			volume,
			playbackRate,
			pan,
...
function playSamples(
	...
	if (audioContext.state != 'running') {
		...
		audioContext
			.resume()
			.then(() => playSamples(sampleChannels, volume, rate, pan, loop, sampleRate, gainNode));
...
function zzfxM(instruments, patterns, sequence, BPM = 125) {
	...
	for (; hasMore; channelIndex++) {
		...
		sequence.forEach((patternIndex, sequenceIndex) => {
			...
			nextSampleOffset =
				outSampleOffset +
				(patterns[patternIndex][0].length - 2 - (notFirstBeat ? 0 : 1)) * beatLength;
...
class TileLayer extends EngineObject {
	...
	getData(layerPos) {
		return (
			layerPos.arrayCheck(this.size) && this.data[((layerPos.y | 0) * this.size.x + layerPos.x) | 0]
		);
	}
	...
	drawTileData(layerPos, clear = true) {
		...
		if (d.tile != undefined) {
			const pos = this.pos.add(layerPos).add(vec2(0.5));
			ASSERT(mainContext == this.context, 'must call redrawStart() before drawing tiles');
...
function engineInit(
	...
	(glCanvas || mainCanvas).style.cssText =
...
function engineObjectsCollect(pos, size, objects = engineObjects) {
	...
	if (!pos) {
		...
		// bounding box test
		for (const o of objects) isOverlapping(pos, size, o.pos, o.size) && collectedObjects.push(o);
	} // circle test
	else {
...
function engineObjectsRaycast(start, end, objects = engineObjects) {
	...
	debugRaycast && debugLine(start, end, hitObjects.length ? '#f00' : '#00f', 0.02);
	return hitObjects;
}
...
function drawEngineSplashScreen(t) {
	...
	const color = (c = 0, l = 0) =>
		hsl([0.98, 0.3, 0.57, 0.14][c % 4] - 10, 0.8, [0, 0.3, 0.5, 0.8, 0.9][l]).toString();
